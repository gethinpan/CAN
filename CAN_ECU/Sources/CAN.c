...
#define ACC_CAN0_EX_ID ConvertID2MSCAN_ID(ID_CAN0)//|(0X00180000)&(0xFFFFFFFE))//È·±£Ö¡ÀàÐÍÎªÀ©Õ¹Êý¾ÝÖ¡
...
#define ACC_CAN4_EX_ID ConvertID2MSCAN_ID(ID_CAN4)//|(0X00180000)&(0xFFFFFFFE))
...
-----------------------------------------------------------------*/
int CAN0SendFrame(unsigned long id, unsigned char priority, unsigned char length, unsigned char *data)
{    
  ...
  *((unsigned long *) ((unsigned long)(&CAN0TXIDR0)))= id;
  ...
  {
     *(&CAN0TXDSR0 + index) = data[index];   //  Load data to Tx buffer  Data Segment Registers(ONLY 8 BYTES?)                                                   
  }
...
-----------------------------------------------------------------*/
int CAN4SendFrame(unsigned long id, unsigned char priority, unsigned char length, unsigned char *data )
{    
  unsigned char index;                          // number for read message
  ...
  *((unsigned long *) ((unsigned long)(&CAN4TXIDR0)))= id;//½ÓÊÕ·½
  ...
  for (index=0;index<length;index++) 
  {
     *(&CAN4TXDSR0 + index) = data[index];   //  Load data to Tx buffer  Data Segment Registers(ONLY 8 BYTES?)                                                   
  }
